...
function DetailPage() {
  ...
  const { data: species } = useFetch(pokemon?.species?.url ?? null);
      ...
      <div className="bg-[#1e1e24] border border-gray-800 rounded-2xl shadow p-6 mb-4 flex flex-col sm:flex-row items-center gap-6">
        ...
        <div className="flex-1 w-full">
          <div className="flex items-center justify-between mb-1">
            ...
            <FavoriteButton
              pokemon={{ id: String(pokemon.id), name: pokemon.name }}
              compact
            />
          </div>
      ...
      <Section title={`Movimientos (primeros ${movimientos.length})`}>
        {movimientos.length > 0 ? (
          ...
          <p className="text-sm text-gray-400 italic">
            La PokéAPI no tiene registrados movimientos específicos para esta
            forma.
          </p>
        )}
      </Section>
