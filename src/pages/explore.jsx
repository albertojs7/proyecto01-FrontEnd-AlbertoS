...
function ExplorePage() {
  ...
  const fetchUrl = isSearching ? null
    : hasTypeFilter
  ...
  const totalPages = hasTypeFilter || isSearching
    ? 1
  ...
  const globalResults = globalSearch && allPokemon.length > 0
    ...
    <main className="max-w-5xl mx-auto px-4 py-8">
      ...
      {!isSearching && (
        <div className="flex flex-wrap items-end justify-center gap-4 mb-8">
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="local-search" className="text-xs text-gray-500 px-1">
              En esta página
            </label>
            <input
              id="local-search"
          ...
          {globalResults.length === 0 ? (
            ...
            <>
              <p className="text-sm text-gray-400 mb-4 text-center">
                {globalResults.length} resultado{globalResults.length !== 1 ? 's' : ''} para "{globalSearch}"
              </p>
