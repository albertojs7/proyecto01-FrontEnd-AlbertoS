...
function EvoCard({ id, name, isCurrent }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-xs font-semibold capitalize text-gray-700">{name}</span>
      <span className="text-xs text-gray-400">#{String(id).padStart(3, '0')}</span>
    </Link>
  )
}
...
function Arrow({ label }) {
  return (
    <div className="flex flex-col items-center gap-1 text-gray-400 px-1" aria-label={label ?? ''}>
      <svg
...
function EvolutionChain({ evolutionUrl, currentId }) {
  ...
  if (levels.length <= 1) return (
    <p className="text-sm text-gray-400 text-center py-2">Este pokémon no evoluciona.</p>
  )
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      {levels.map((level, levelIndex) => {
        ...
        return (
          <div key={levelIndex} className="flex flex-col items-center gap-3 w-full">
